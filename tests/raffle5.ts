...
describe("pnft_transfer tests (end raffle 5)", () => {
  // Configure the client to use the local cluster.
  //const CURRENT_RAFFLE = 5;
  ...
  for(let CURRENT_RAFFLE = 5; CURRENT_RAFFLE <= 20; CURRENT_RAFFLE++){
  ...
  it("Checks that the raffle 5 is active", async () => {
    const raffleStatus = await program.account.raffle.fetch(rafflePda);
    assert.ok(raffleStatus.active, "raffle 5 is active");
  });
  it("Checks that the tracker is updated for raffle 2", async () => {
    const raffleTracker = await program.account.raffleTracker.fetch(trackerPda);
    assert.ok(raffleTracker.currentRaffle.eq(new anchor.BN(CURRENT_RAFFLE)), "currentRaffle should be 5");
  });
