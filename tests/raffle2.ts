...
describe("pnft_transfer tests (end raffle 2)", () => {
  // Configure the client to use the local cluster.
  const CURRENT_RAFFLE = 2;
  ...
  it("Checks that the raffle2 is active", async () => {
  ...
  it("Checks that the tracker is updated for raffle 2", async () => {
    ...
    assert.ok(raffleTracker.currentRaffle.eq(new anchor.BN(CURRENT_RAFFLE)), "currentRaffle should be 2");
  });
