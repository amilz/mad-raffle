...
describe("pnft_transfer tests (end raffle 3)", () => {
  // Configure the client to use the local cluster.
  const CURRENT_RAFFLE = 3;
  ...
  it("Checks that the raffle3 is active", async () => {
    const raffleStatus = await program.account.raffle.fetch(rafflePda);
    assert.ok(raffleStatus.active, "raffle 3 is active");
  });
  it("Checks that the tracker is updated for raffle 3", async () => {
    const raffleTracker = await program.account.raffleTracker.fetch(trackerPda);
    assert.ok(raffleTracker.currentRaffle.eq(new anchor.BN(CURRENT_RAFFLE)), "currentRaffle should be 3");
  });
